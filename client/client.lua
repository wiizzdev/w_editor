            ...
            while open do 
                RageUI.IsVisible(mainMenu, function()
                    ...
                    if not (IsRecording()) then
                        RageUI.Button("Lancer l'Enregistrement", nil, {RightLabel = "→"}, true, {
                            onSelected = function() StartRecording(1) end
                        })
                    else
                    ...
                    if(IsRecording()) then
                        RageUI.Button("Arrêter l'Enregistrement", nil, {RightLabel = "→"}, true, {
                            onSelected = function() StopRecordingAndSaveClip() end
                        })
                    else
                ...
                RageUI.IsVisible(confirmation, function()
                    ...
                    RageUI.Button("Oui", nil, {RightLabel = "→"}, true, {
                        onSelected = function() NetworkSessionLeaveSinglePlayer() ActivateRockstarEditor() end
                    })
                    RageUI.Button("Non", nil, {RightLabel = "→"}, true, {
                        onSelected = function() RageUI.GoBack() end
                    })
                end)
            Wait(0)
            end
